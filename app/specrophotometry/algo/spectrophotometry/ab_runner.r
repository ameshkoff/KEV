...
ab.evaluation.runner <- function(mode = c("api", "script", "app")
  ...
  if (mode %in% c("script", "api")) {
    ...
    dir.start.eq <- "app/kev/algo/"
    dir.start <- "app/spectrophotometry/algo/"
    
  } else {
    
    dir.start.eq <- "../kev/algo/"
    dir.start <- "algo/"
    
  }
  
  source(paste0(dir.start.eq, "concentrations/eq_data.r"), chdir = TRUE)
  source(paste0(dir.start, "spectrophotometry/ab_data.r"), chdir = TRUE)
  
  source(paste0(dir.start.eq, "concentrations/eq_preproc.r"), chdir = TRUE)
  source(paste0(dir.start, "spectrophotometry/ab_preproc.r"), chdir = TRUE)
  
  source(paste0(dir.start.eq, "concentrations/eq_evaluator.r"), chdir = TRUE)
  source(paste0(dir.start, "spectrophotometry/ab_evaluator.r"), chdir = TRUE)
  
  source(paste0(dir.start.eq, "concentrations/eq_postproc.r"), chdir = TRUE)
...
ab.evaluation.grid <- function(mode = c("api", "script", "app")
  ...
  if (mode %in% c("script", "api")) {
    ...
    dir.start.eq <- "app/kev/"
    dir.start <- "app/spectrophotometry/"
    
  }

  source(paste0(dir.start.eq, "eq_data.r"), chdir = TRUE)
  source(paste0(dir.start, "ab_data.r"), chdir = TRUE)
  
  source(paste0(dir.start.eq, "eq_preproc.r"), chdir = TRUE)
  source(paste0(dir.start, "ab_preproc.r"), chdir = TRUE)
  
  source(paste0(dir.start.eq, "eq_evaluator.r"), chdir = TRUE)
  source(paste0(dir.start, "ab_evaluator.r"), chdir = TRUE)
  
  source(paste0(dir.start.eq, "eq_postproc.r"), chdir = TRUE)
