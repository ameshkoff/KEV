...
ab.save <- function(subdir, sep, dt.res, dt.ab.calc, ab.res.abs, ab.res.rel, ab.err, cnst.dev, cor.m, mol.coef, mol.coef.dev, err.diff, cnst.tune) {
  ...
  if (sep == ";") {
    ...
    write.csv2(scl, file = paste0("output", subdir, "fmin_last_diff.csv"), row.names = FALSE)
    ...
    write.csv(scl, file = paste0("output", subdir, "fmin_last_diff.csv"), row.names = FALSE)
    ...
    write.table(scl, file = paste0("output", subdir, "fmin_last_diff.csv"), sep = "\t", row.names = FALSE)
